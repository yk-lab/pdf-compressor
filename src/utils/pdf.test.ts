...
describe('PDF Utilities', () => {
  ...
  describe('createCompressedPdfFromImages', () => {
    it('should throw PDFCompressionSizeError when cannot compress below limit', async () => {
      // Create a minimal valid JPEG for testing (1x1 black pixel)
      ...
      // Create large blob to simulate file too large scenario
      const largeBlob = new Blob([validJpegData], { type: 'image/jpeg' });
      // Add large padding to simulate a large file
      const largePadding = new Uint8Array(200000).fill(0);
      const largeArrayBuffer = new Uint8Array(validJpegData.length + largePadding.length);
      largeArrayBuffer.set(validJpegData, 0);
      largeBlob.arrayBuffer = vi.fn().mockResolvedValue(largeArrayBuffer.buffer);
  ...
  describe('mergePdfFiles', () => {
    ...
    it('should handle single image file and compress large images', async () => {
      ...
      // Create valid JPEG data for the mock blob
      const validJpegData = new Uint8Array([
        ...
        0x00, 0x00, 0x00, 0x0a, 0xff, 0xc4, 0x00, 0x14, 0x10, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff, 0xda, 0x00, 0x08,
        0x01, 0x01, 0x00, 0x00, 0x3f, 0x00, 0x54, 0x50, 0xff, 0xd9,
      ]);
